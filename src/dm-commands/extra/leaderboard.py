...
class Command:
    ...
    async def execute(self, user_id, conversation_id, message: str):
        ...
        try:
            await self.bot.highrise.send_message(conversation_id, "------------\nBANK HEIST STATS\n" + 
                                f"\n".join(combined) +"\n------------")       
            await self.bot.highrise.send_message(conversation_id, f"{total_winners} out of {len(data)} players have completed at least one heist successfully! Players have been caught {total_losses} times combined.") 
